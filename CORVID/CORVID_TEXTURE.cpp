...
void CORVID_TEXTURE::initializeTextures(std::vector<path>* imgfiles) {
	...
	global_textureList = new std::vector<SDL_Surface*>(); // Also, the first line of the method may be able to prevent memory leaks if there are multiple worlds
	for (path i : *imgfiles) {
		global_textureList->push_back(IMG_Load(i.string().c_str())); // For some reason, I need to convert it to a string, and then a C string
	}
};
void CORVID_TEXTURE::rendertext(SDL_Surface* surface, SDL_Rect* offset) { // Will need to make more complicated as I create more complicated textures
